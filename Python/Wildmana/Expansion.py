...
def reqSpreadDiscontent(caster):
	pCity = caster.plot().getPlotCity()
...
def spellSpreadDiscontent(caster):
	pCity = caster.plot().getPlotCity()
	pPlayer = gc.getPlayer(pCity.getOwner())

	iMissionStrength = 2 + caster.getLevel()

	iUnhappy = 2 + CyGame().getSorenRandNum(iMissionStrength, "SpreadDiscontentEffect")
	pCity.changeEspionageHappinessCounter(iUnhappy)
	CyInterface().addMessage(caster.getOwner(),true,25,"Your Enforcer managed to spread discontent among the populace! Unhappiness in the City increased by %d" %(iUnhappy),'',0,'',ColorTypes(11), caster.getX(), caster.getY(), True,True)
...
def reqStealGold(caster):
	...
	if gc.getPlayer(pCity.getOwner()).getGold() < goldToSteal(caster):
		return false
...
def reqSabotageRitual(caster):
	...
	iBuildingClass = gc.getInfoTypeForString("BUILDINGCLASS_MAGEGUILD")
...
def spellEsusHeal(caster,amount):
	pPlot = caster.plot()
	iPoisoned = gc.getInfoTypeForString('PROMOTION_POISONED')
	caster.setHasPromotion(iPoisoned,False)
	if caster.isAlive():
...
def spellDiscoverMiningResource(caster):
	...
	iRnd = CyGame().getSorenRandNum(6,"Pick Bonus")
...
def	spellIncreaseInfluenceOnHolyCity(caster):
	...
	pPlayer.changeFaith(2500)

def	reqAddToArtCollection(caster):
	pCity=caster.plot().getPlotCity()
	if pCity.getNumRealBuilding(gc.getInfoTypeForString('BUILDING_CHATEAU_LIVING_ART')) > 0:
		return true
	return false

def	spellAddToArtCollection(caster):
	iPlayer=caster.getOwner()
	pPlayer = gc.getPlayer(iPlayer)
	pPlayer.changeGlobalCulture(30)
