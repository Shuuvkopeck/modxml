...
def assignStartingPlots():	
	...
	MAX_X = gameMap.getGridWidth()
	MAX_Y = gameMap.getGridHeight()
	iMinX=-1
	iMinY=-1
	iMaxX=-1
	iMaxY=-1		
	for i in range(iPlayers):
		#TODO: Decide which Plane Player should start on
		#Find an open spot on that plane
		if i==0:
			iMinX=2*MAX_X/8
			iMaxX=7*MAX_X/16
			iMinY=2*MAX_Y/8
		...
		if i==3:
			...
			iMinY=9*MAX_Y/16
			iMaxY=6*MAX_Y/8			
		iX=iMinX+CyGame().getMapRandNum(iMaxX-iMinX,'find Starting Plot X')
		iY=iMinY+CyGame().getMapRandNum(iMaxY-iMinY,'find Starting Plot X')
		gc.getPlayer(i).setStartingPlot(gameMap.plot(iX,iY),true)	
