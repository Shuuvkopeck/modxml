		...
		if pPlayer.isHuman() == False:
			...
			if eUnitClass == gc.getInfoTypeForString('UNITCLASS_SCOUT'):
				if pPlayer.getUnitClassCountPlusMaking(gc.getInfoTypeForString('UNITCLASS_SCOUT')) >= 1:
					return True
			if eUnitClass == gc.getInfoTypeForString('UNITCLASS_LIGHTBRINGER'):
				return True
...
# Scions
	...
	def cannotConstruct(self,argsList):
		...
		if pPlayer.isHuman() == False:
			...
			if iBuildingClass == gc.getInfoTypeForString('BUILDINGCLASS_TEMPLE_DISTRICT'):
				return True
			if iBuildingClass == gc.getInfoTypeForString('BUILDINGCLASS_BARD_DISTRICT'):
				return True
			if iBuildingClass == gc.getInfoTypeForString('BUILDINGCLASS_SCHOLA_ARCANA') and pPlayer.getCivilizationType() != gc.getInfoTypeForString('CIVILIZATION_ARISTRAKH'):
				return True
...
#Aristrakh
		...
		#Catacombs, Vampire Palace or Necropolis requires either circle of despair and circle of blood
...
#Austrin
		if eProject == gc.getInfoTypeForString('PROJECT_PREPARE_EXPEDITION'):
			if not pPlayer.getCivilizationType()==gc.getInfoTypeForString('CIVILIZATION_AUSTRIN'):
				return True
