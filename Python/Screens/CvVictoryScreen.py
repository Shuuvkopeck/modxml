...
class CvVictoryScreen:
	...
	def __init__(self, screenId):
		...
		self.TABLE_WIDTH_0 = 250
		self.TABLE_WIDTH_1 = 330
		self.TABLE_WIDTH_2 = 130
		self.TABLE_WIDTH_3 = 100
		self.TABLE_WIDTH_4 = 110
		self.TABLE_WIDTH_5 = 80
	...
	def showVictoryConditionScreen(self):
		...
		for iLoopVC in range(gc.getNumVictoryInfos()):
			...
			if gc.getGame().isVictoryValid(iLoopVC):
				...
				#Special Rules for Luonnotar Display  	#xtended
...
#							screen.setTableText(szTable, 5, iRow, gc.getTeam(iBestBuildingTeam).getBuildingClassCount(i), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
	...
	#							screen.setTableText(szTable, 5, iRow, gc.getTeam(iBestBuildingTeam).getBuildingClassCount(i), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
				...
				if (iLoopVC == gc.getInfoTypeForString("VICTORY_PRESTIGE")):

					iRow = screen.appendTableRow(szTable)											
					screen.setTableText(szTable, 2, iRow, gc.getPlayer(self.iActivePlayer).getName() + ":", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
					screen.setTableText(szTable, 3, iRow, (u"%d" % gc.getPlayer(self.iActivePlayer).getPrestige()), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)										
				
					for iLoopPlayer in range(gc.getMAX_PLAYERS()):
						if (gc.getPlayer(iLoopPlayer).isAlive() and not gc.getPlayer(iLoopPlayer).isMinorCiv() and not gc.getPlayer(iLoopPlayer).isBarbarian()):
							if (iLoopPlayer != self.iActivePlayer and (activePlayer.getTeam().isHasMet(gc.getPlayer(iLoopPlayer).getTeam()) or gc.getGame().isDebugMode())):					
								iRow = screen.appendTableRow(szTable)											
								screen.setTableText(szTable, 2, iRow, gc.getPlayer(iLoopPlayer).getName() + ":", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
								screen.setTableText(szTable, 3, iRow, (u"%d" % gc.getPlayer(iLoopPlayer).getPrestige()), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
										
					bEntriesFound = True

					
				if (iLoopVC==gc.getInfoTypeForString("VICTORY_EPIC_ADVENTURE")):
					bEntriesFound = true
					iRow = screen.appendTableRow(szTable)				
					pPlayer=gc.getPlayer(self.iActivePlayer)
					for i in range(gc.getNumAdventureInfos()):
						if(gc.getAdventureInfo(i).isVictory() and pPlayer.isAdventureEnabled(i)):
							iRow = screen.appendTableRow(szTable)				
							screen.setTableText(szTable, 0, iRow, gc.getAdventureInfo(i).getDescription(), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
							if (pPlayer.isAdventureFinished(i)):
								screen.setTableText(szTable, 3, iRow, localText.getText("TXT_KEY_VICTORY_SCREEN_EPIC_ADVENTURE_FINISHED", ()), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)							
							...
							for j in range(gc.getMAX_ADVENTURESTEPS()):
								if gc.getAdventureInfo(i).getAdventureStep(j)>=0 and gc.getAdventureInfo(i).getAdventureStep(j)<gc.getNumAdventureStepInfos():
									...
									iStatus =pPlayer.getAdventureStatus(i,j)
									iLimit = pPlayer.getAdventureLimit(i,j)
									screen.setTableText(szTable, 1, iRow, localText.getText(textkey, (iStatus,iLimit)), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)																

									if (iStatus>=iLimit):
										screen.setTableText(szTable, 3, iRow, localText.getText("TXT_KEY_VICTORY_SCREEN_EPIC_ADVENTURE_FINISHED", ()), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)							
									else:
										screen.setTableText(szTable, 3, iRow, localText.getText("TXT_KEY_VICTORY_SCREEN_EPIC_ADVENTURE_UNFINISHED", ()), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
										
						if(gc.getAdventureInfo(i).isVictoryFailed() and pPlayer.isAdventureEnabled(i)):
							iRow = screen.appendTableRow(szTable)				
							screen.setTableText(szTable, 0, iRow, gc.getAdventureInfo(i).getDescription(), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
							if (pPlayer.isAdventureFinished(i)):
								screen.setTableText(szTable, 3, iRow, localText.getText("TXT_KEY_VICTORY_SCREEN_EPIC_ADVENTURE_FINISHED", ()), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)							
							...
							for j in range(gc.getMAX_ADVENTURESTEPS()):
								if gc.getAdventureInfo(i).getAdventureStep(j)>=0 and gc.getAdventureInfo(i).getAdventureStep(j)<gc.getNumAdventureStepInfos():
									...
									if (iStatus>=iLimit):
										screen.setTableText(szTable, 3, iRow, localText.getText("TXT_KEY_VICTORY_SCREEN_EPIC_ADVENTURE_FAILED", ()), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
									else:
										screen.setTableText(szTable, 3, iRow, localText.getText("TXT_KEY_VICTORY_SCREEN_EPIC_ADVENTURE_OK", ()), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)																
