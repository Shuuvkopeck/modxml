...
#added Sephi	
		...
		self.TXT_KEY_DICT = {
			"UNITS": "TXT_KEY_SCREEN_RESEARCH_MILITARY",
			"MAGICANDRELIGION": "TXT_KEY_SCREEN_RESEARCH_RELIGION_MAGIC",
			"ECONOMY": "TXT_KEY_SCREEN_SPELL_RESEARCH_ECONOMY",
			}

		self.ORDER_LIST = ["UNITS", \
											 "MAGICANDRELIGION", \
											 "ECONOMY"]											 

		self.iDefaultScreen = self.SCREEN_DICT["UNITS"]
	...
	def placeTechs (self):
		...
		for i in range (len (self.ORDER_LIST)):
			...
			if (self.iScreen != self.SCREEN_DICT[szScreen]):
				screen.setText (szTextId, "", u"<font=4>" + localText.getText (self.TXT_KEY_DICT[szScreen], ()).upper() + u"</font>", CvUtil.FONT_CENTER_JUSTIFY, xLink, self.Y_LINK, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_TECH_SCREEN, self.SCREEN_DICT[szScreen], -1)
			else:
				screen.setText (szTextId, "", u"<font=4>" + localText.getColorText (self.TXT_KEY_DICT[szScreen], (), gc.getInfoTypeForString ("COLOR_YELLOW")).upper() + u"</font>", CvUtil.FONT_CENTER_JUSTIFY, xLink, self.Y_LINK, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_TECH_SCREEN, -1, -1)
			xLink += self.DX_LINK
		...
		for i in range(gc.getNumTechInfos()):
			...
			# optimization: techs with state '-1' will be ignored instead of placed outside the screen
			if gc.getTechInfo(i).getSpellSchool()!=-1 or gc.getTechInfo(i).isDisable() or gc.getTechInfo(i).getFlavorValue(FlavorTypes.FLAVOR_GUILD)>0:
			...
			if self.iScreen==0:
				if gc.getTechInfo(i).getFlavorValue(FlavorTypes.FLAVOR_MELEE)>0:
					bTechValid=true			
				elif gc.getTechInfo(i).getFlavorValue(FlavorTypes.FLAVOR_RANGED)>0:
					bTechValid=true			
				elif gc.getTechInfo(i).getFlavorValue(FlavorTypes.FLAVOR_RECON)>0:
					bTechValid=true			
				elif gc.getTechInfo(i).getFlavorValue(FlavorTypes.FLAVOR_MOUNTED)>0:
					bTechValid=true			
			elif self.iScreen==1:
				if gc.getTechInfo(i).getFlavorValue(FlavorTypes.FLAVOR_MAGIC)>0:
					bTechValid=true			
				elif gc.getTechInfo(i).getFlavorValue(FlavorTypes.FLAVOR_RELIGION)>0:
					bTechValid=true			
			elif self.iScreen==2:		
				bTechValid=true						
				if gc.getTechInfo(i).getFlavorValue(FlavorTypes.FLAVOR_MELEE)>0:
					...
					bTechValid=false			
				elif gc.getTechInfo(i).getFlavorValue(FlavorTypes.FLAVOR_MAGIC)>0:
					bTechValid=false			
				elif gc.getTechInfo(i).getFlavorValue(FlavorTypes.FLAVOR_RELIGION)>0:
					bTechValid=false			
