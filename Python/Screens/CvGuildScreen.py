...
#added Sephi	
		...
		self.DRAW_DICT = {
			"GUILDS": self.drawGuilds,
			"TRAITS": self.drawTraits,
			}			
			
		self.TXT_KEY_DICT = {
			"GUILDS": "TXT_KEY_SCREEN_RESEARCH_GUILDS",
			"TRAITS": "TXT_KEY_SCREEN_RESEARCH_TRAITS",			
			}

		self.ORDER_LIST = ["GUILDS", "TRAITS"]											 

		self.iDefaultScreen = self.SCREEN_DICT["TRAITS"]
...
#added Sephi									
	...
	def hideScreen (self):
		...
		# Get the screen
		screen = CyGInterfaceScreen( "Guilds", CvScreenEnums.GUILD_SCREEN )
	...
	def getScreen(self):
		return CyGInterfaceScreen( "Guilds", CvScreenEnums.GUILD_SCREEN)
...
#		screen.setDimensions(screen.centerX(0), screen.centerY(0), 1024, 768)
		...
		if CityScreenOpt.isDisableFlavor():							
			...
			screen.addDDSGFC("TechBG", ArtFileMgr.getInterfaceArtInfo("SCREEN_BG").getPath(), 0, 48, self.W_SCREEN, self.H_SCREEN-100, WidgetTypes.WIDGET_GENERAL, -1, -1 )
			screen.addPanel( "TechBottomPanel", u"", u"", True, False, 0, self.H_SCREEN-60, self.W_SCREEN, 55, PanelStyles.PANEL_STYLE_BOTTOMBAR )
		else:
			...
			szArt=str(gc.getCivilizationInfo(iCiv).getFlavorScreenImage())
			szArt=szArt+"_back.dds"
			screen.addDrawControl( "BackgroundFlavor", szArt, 0, 0, xResolution, yResolution, WidgetTypes.WIDGET_GENERAL, -1, -1 )						
		...
		screen.addScrollPanel( "TechList", u"", 0, 64, self.W_SCREEN, self.H_SCREEN-140, PanelStyles.PANEL_STYLE_EXTERNAL )
		...
		for i in range (len (self.ORDER_LIST)):
			...
			if (self.iScreen != self.SCREEN_DICT[szScreen]):
				screen.setText (szTextId, "", u"<font=4>" + localText.getText (self.TXT_KEY_DICT[szScreen], ()).upper() + u"</font>", CvUtil.FONT_CENTER_JUSTIFY, xLink, self.Y_LINK, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GUILD_SCREEN, self.SCREEN_DICT[szScreen], -1)
			else:
				screen.setText (szTextId, "", u"<font=4>" + localText.getColorText (self.TXT_KEY_DICT[szScreen], (), gc.getInfoTypeForString ("COLOR_YELLOW")).upper() + u"</font>", CvUtil.FONT_CENTER_JUSTIFY, xLink, self.Y_LINK, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GUILD_SCREEN, -1, -1)
			xLink += self.DX_LINK		
	...
	def drawTraits (self):	
		...
		pPlayer = gc.getPlayer(gc.getGame().getActivePlayer())
		iCulture = pPlayer.getGlobalCulture()
		iCapitalID = -1 		
		...
		for i in range(gc.getNumTraitInfos()):
			if pPlayer.canGetTrait(i):
				szDescription = gc.getTraitInfo(i).getDescription()
# Display Culture Cost				
#				szDescription = szDescription + "("+str(pPlayer.getMagicRitualTurnsNeeded(i,false))+" Turns)"

# todo: use cost formula
				if pPlayer.getTraitCost(i) > iCulture:
				...
				screen.appendListBoxString(self.SUMMON_LIST_ID, szDescription, WidgetTypes.WIDGET_DO_TRAIT_FROM_CULTURE, i, iCapitalID, CvUtil.FONT_LEFT_JUSTIFY)
	...
	def placeTechs (self):
		...
		iCapitalID = gc.getPlayer(gc.getGame().getActivePlayer()).getCapitalCity().getID()
		...
		for i in range(gc.getNumTechInfos()):
			...
			if gc.getTechInfo(i).getFlavorValue(FlavorTypes.FLAVOR_GUILD) < 1:
			...
			screen.attachPanelAt( "TechList", szTechRecord, u"", u"", True, False, PanelStyles.PANEL_STYLE_TECH, iX - 6, iY - 6, self.getXStart() + 6, 12 + ( BOX_INCREMENT_HEIGHT * PIXEL_INCREMENT ), WidgetTypes.WIDGET_CULTURE_ACHIEVEMENT, i, iCapitalID )
			...
			elif (gc.getPlayer(self.iCivSelected).canGetCulturalAchievement(i)):
...
#FfH: End Modify
			...
			for j in range( gc.getNumSpellInfos() ):
				if (gc.getSpellInfo(j).getTechPrereq() == i):
					if gc.getSpellInfo(j).getCivilizationPrereq()==-1:
						szSpellInfo = "SpellInfoButton" + str( ( i * 1000 ) + j )
						screen.addDDSGFCAt( szSpellInfo, szTechRecord, gc.getSpellInfo(j).getButton(), iX + fX, iY + Y_ROW, TEXTURE_SIZE, TEXTURE_SIZE, WidgetTypes.WIDGET_PEDIA_JUMP_TO_SPELL, j, 1, False )
						fX += X_INCREMENT
	...
	def updateTechRecords (self, bForce):
		...
		for i in range(gc.getNumTechInfos()):
			...
			if ( gc.getTeam(gc.getPlayer(self.iCivSelected).getTeam()).isHasTech(i) ):
				if ( self.aiCurrentState[i] != CIV_HAS_TECH ):
					...
					abChanged[i] = 1
					bAnyChanged = 1
			elif ( not gc.getPlayer(self.iCivSelected).canGetCulturalAchievement(i)):
		...
		for i in range(gc.getNumTechInfos()):
			...
			if (abChanged[i] or bForce or (bAnyChanged and gc.getPlayer(self.iCivSelected).isResearchingTech(i))):
				...
				if (self.aiCurrentState[i] == CIV_HAS_TECH):
					screen.setPanelColor(szTechRecord, 85, 150, 87)
				elif (self.aiCurrentState[i] == CIV_IS_RESEARCHING):
					screen.setPanelColor(szTechRecord, 104, 158, 165)
				else:
					screen.setPanelColor(szTechRecord, 206, 65, 69)
	...
	def drawArrows (self):
		...
		for i in range(gc.getNumTechInfos()):
			...
			if gc.getTechInfo(i).getFlavorValue(FlavorTypes.FLAVOR_GUILD)>0:
				bTechValid=true			
			...
			if bTechValid:
				...
				for j in range( gc.getNUM_AND_TECH_PREREQS() ):
					...
					if ( eTech > -1 ):
						...
						if gc.getTechInfo(i).getSpellSchool()!=-1 or gc.getTechInfo(i).isDisable():
							iX=-2000
							iY=-2000
						
						if gc.getTechInfo(i).isDrawNoAndPrereqArrows():
							iX=-2000
							iY=-2000
		...
		#					screen.addDDSGFCAt( szTechPrereqID, "TechList", gc.getTechInfo(eTech).getButton(), iX + fX, iY + 6, TEXTURE_SIZE, TEXTURE_SIZE, WidgetTypes.WIDGET_HELP_TECH_PREPREQ, eTech, -1, False )
				...
				for j in range( gc.getNUM_OR_TECH_PREREQS() ):
					...
					if ( eTech > -1 ):
						...
						if gc.getTechInfo(i).isDrawNoOrPrereqArrows():
							iX=-2000
							iY=-2000
