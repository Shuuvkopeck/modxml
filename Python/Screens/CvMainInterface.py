...
#Civ Counters
		...
		if (not CyInterface().isCityScreenUp() and CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_HIDE_ALL and CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_ADVANCED_START and CyInterface().getShowInterface() == InterfaceVisibility.INTERFACE_SHOW):
			...
			#Elohim Purity display
			if pPlayer.getCivilizationType() == gc.getInfoTypeForString('CIVILIZATION_ELOHIM'):

			#figuring spawn chance
				iPurity = pPlayer.getPurityCounter()
			#/figuring spawn chance
				if iPurity > -1:

					SRstr = u"<font=2i>%s%i%s</font>" %(str("Elohim Purity Counter: "),(iPurity),str("% "))
					screen.setImageButton("ElohimPurity", "Art/Interface/Buttons/Promotions/ShieldofFaith.dds", 20, 200, 16, 16, WidgetTypes.WIDGET_HELP_CIVCOUNTER_ELOHIM, -1, -1 )
					screen.setText( "PurText", "Background", SRstr, CvUtil.FONT_LEFT_JUSTIFY, 35, 200, 0.5, FontTypes.GAME_FONT, WidgetTypes.WIDGET_HELP_CIVCOUNTER_ELOHIM, -1, -1 )
#					screen.setHitTest( "PurText", HitTestTypes.HITTEST_NOHIT )
				else:
					screen.hide( "ElohimPurity" )
					screen.hide( "PurText" )
...
#						xCoord = screen.centerX(512)
					...
					if gc.getPlayer(ePlayer).getSpellResearchFocus()!=-1:
						...
						if gc.getPlayer(ePlayer).getArcaneIncome()>0:
							iTurnsNeeded = iTurnsNeeded / gc.getPlayer(ePlayer).getArcaneIncome()
