...
#added Sephi	
	...
	def setValues(self):
		...
		self.HEADINGS_WIDTH = self.W_SCREEN / 6
		self.X_EXIT = 994+screen.centerX(0)
		self.Y_EXIT = 726+screen.centerY(0)
		self.BOTTOM_LINE_WIDTH = screen.getXResolution() - 10
		self.X_CANCEL = 552+screen.centerX(0)
		self.Y_CANCEL = 726+screen.centerY(0)
				
		self.Y_TITLE = 8+screen.centerY(0)		
		
		self.HEADINGS_TOP = 50+screen.centerY(0)		
		self.HEADINGS_BOTTOM = 307+screen.centerY(0)		
		
		self.HELP_TOP = 375+screen.centerY(0)		
		self.HELP_BOTTOM = 638+screen.centerY(0)	
		
		self.BOTTOM_LINE_TOP = 650+screen.centerY(0)		
		
#		self.SPACE_X = screen.centerX(0)
...
#		screen.setDimensions(screen.centerX(0), screen.centerY(0), self.W_SCREEN, self.H_SCREEN)
		...
		if CityScreenOpt.isDisableFlavor():					
			...
			screen.addPanel( "TechBottomPanel", u"", u"", True, False, 0, 713+screen.centerY(0)	, self.W_SCREEN, 55, PanelStyles.PANEL_STYLE_BOTTOMBAR )
		else:		
		...
		#Added Space_x Sephi
		screen.addPanel("CivicsBottomLine", "", "", True, True, self.SPACE_X + self.HEADINGS_SPACING, self.BOTTOM_LINE_TOP, self.BOTTOM_LINE_WIDTH, self.BOTTOM_LINE_HEIGHT, PanelStyles.PANEL_STYLE_MAIN)
	...
	def drawAllButtons(self):				
		...
		for i in range(gc.getNumCivicOptionInfos()):
			...
			#added Space_x Sephi
#			fX = self.SPACE_X + self.HEADINGS_SPACING  + (self.HEADINGS_WIDTH + self.HEADINGS_SPACING) * i
			fX = self.W_SCREEN / gc.getNumCivicOptionInfos() * i
...
#			screen.setLabel("", "Background",  u"<font=3>" + gc.getCivicOptionInfo(i).getDescription().upper() + u"</font>", CvUtil.FONT_CENTER_JUSTIFY, fX + self.HEADINGS_WIDTH/2, self.HEADINGS_TOP + self.TEXT_MARGIN, 0, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )
			...
			for j in range(gc.getNumCivicInfos()):
				if (gc.getCivicInfo(j).getCivicOptionType() == i):										
					if(gc.getCivicInfo(j).getPrereqCivilization() == -1 or gc.getCivicInfo(j).getPrereqCivilization() == activePlayer.getCivilizationType()):
						if(gc.getCivilizationInfo(activePlayer.getCivilizationType()).isCivicAllowed(j)):
...
#FfH: End Add
	...
	def drawHelpText(self, iCivicOption):
		...
		#added Space X Sephi
#		fX = self.SPACE_X + self.HEADINGS_SPACING  + (self.HEADINGS_WIDTH + self.HEADINGS_SPACING) * iCivicOption
		fX = self.W_SCREEN / gc.getNumCivicOptionInfos() * iCivicOption
