...
#			lList = lList + ['CRAZED', 'DEMONIC_POSSESSION', 'DISEASED', 'ENRAGED', 'PLAGUED', 'POISONED', 'WITHERED']
	...
	def exploreLairBigGood(self, caster):
		...
		if pPlayer.canReceiveGoody(pPlot, gc.getInfoTypeForString('GOODY_GRAVE_TECH'), caster):
			lList = lList + ['TECH']
		if not pPlot.isWater():
			lList = lList + ['ITEM_ALSOPHOCUS_TOME', 'ITEM_MARTYR_CROWN', 'ITEM_PRESERVER', 'ITEM_SEVEN_LEAGUE', 'ITEM_TIMOR_MASK', 'PRISONER_ADVENTURER', 'PRISONER_ARTIST', 'PRISONER_COMMANDER', 'PRISONER_ENGINEER', 'PRISONER_MERCHANT', 'PRISONER_PROPHET', 'PRISONER_SCIENTIST']
	...
	def doHellTerrain(self):
		# bIsHellHere = not gc.getGame().isOption(GameOptionTypes.GAMEOPTION_NO_PLOT_COUNTER)	
		bIsHellHere = True			
							...
							for iiY in range(iY-1, iY+2, 1):
								...
								if pAdjacentPlot.isNone() == False:
									if pAdjacentPlot.getPlotCounter() > 10:
										if CyGame().getSorenRandNum(100, "Hell vs. Resist Hostile Terraform") > pPlayer.getResistHostileTerraforming():
