...
from unblemishedeventmod import *
from RandomNameUtils import *
...
def doArmageddonApocalypse(argsList):
	...
	for pUnit in apUnitList:
		if (CyGame().getSorenRandNum(100, "Apocalypse") <= iPercent):
			if pUnit.isAlive():
				if not pUnit.isHasPromotion(gc.getInfoTypeForString('PROMOTION_AMULET_OF_LIFE')):
...
def doArmageddonBlight(argsList):
	...
	for pUnit in py.getUnitList():
		if pUnit.isAlive():
			pUnit.doDamageNoCaster(25, 100, gc.getInfoTypeForString('DAMAGE_UNHOLY'), false)
...
def applyExploreLairPortal1(argsList):
	...
	if pBestPlot != -1:
		...
		CyInterface().addMessage(iPlayer,True,25,CyTranslator().getText("TXT_KEY_MESSAGE_EXPLORE_LAIR_PORTAL",()),'',1,', ,Art/Interface/Buttons/Spells/Spells_Atlas1.dds,3,4',ColorTypes(8),pBestPlot.getX(),pBestPlot.getY(),True,True)
...
def canTriggerMutateUnit(argsList):
	...
	if pUnit.isHasPromotion(iMutated) or pUnit.isHasPromotion(iUndead):
		return False
	return True
...
def doPigGiant3(argsList):
	...
	if pPlot != -1:
		bPlayer = gc.getPlayer(gc.getBARBARIAN_PLAYER())
		newUnit = bPlayer.initUnit(gc.getInfoTypeForString('UNIT_HILL_GIANT'), pPlot.getX(), pPlot.getY(), UnitAITypes.UNITAI_ATTACK, DirectionTypes.DIRECTION_SOUTH)
...
def applyShrineCamulos2(argsList):
	...
	if CyGame().getSorenRandNum(100, "Shrine Camulos") < 10:
		pPlot = cf.findClearPlot(-1, pCity.plot())
		if pPlot != -1:
			CyInterface().addMessage(iPlayer,True,25,CyTranslator().getText("TXT_KEY_MESSAGE_SHRINE_CAMULOS",()),'',1,', ,Art/Interface/Buttons/Units/Units_Atlas3.dds,1,15',ColorTypes(8),pCity.getX(),pCity.getY(),True,True)
			bPlayer = gc.getPlayer(gc.getBARBARIAN_PLAYER())
...
def doSignSucellus(argsList):
	...
	for pUnit in apUnitList:
		if pUnit.isHasPromotion(iDiseased):
			...
			CyInterface().addMessage(iPlayer,True,25,CyTranslator().getText("TXT_KEY_MESSAGE_POOL_OF_TEARS_DISEASED",()),'AS2D_FEATUREGROWTH',1,', ,Art/Interface/Buttons/Spells/Spells_Atlas1.dds,8,2',ColorTypes(8),pUnit.getX(),pUnit.getY(),True,True)
		if pUnit.getDamage() > 0:
			pUnit.setDamage(pUnit.getDamage() / 2, PlayerTypes.NO_PLAYER)
			CyInterface().addMessage(iPlayer,True,25,CyTranslator().getText("TXT_KEY_MESSAGE_UNIT_HEALED",()),'AS2D_FEATUREGROWTH',1,', ,Art/Interface/Buttons/Spells/Spells_Atlas1.dds,5,5',ColorTypes(8),pUnit.getX(),pUnit.getY(),True,True)
...
def canTriggerSwitchCivs(argsList):
	...
	if CyGame().getRankPlayer(0) != kTriggeredData.ePlayer:
		return False
	if otherPlayer.getCityStateType() != -1:
		return False
...
def doSlaveEscape(argsList):
	...
	pUnit = pPlayer.getUnit(kTriggeredData.iUnitId)
	pUnit.kill(False, -1)
	CyInterface().addMessage(iPlayer,True,25,CyTranslator().getText("TXT_KEY_MESSAGE_SLAVE_ESCAPE", ()),'',1,', ,Art/Interface/Buttons/Units/Units_atlas4.dds,1,7',ColorTypes(8),pUnit.getX(),pUnit.getY(),True,True)
...
def doSlaveRevolt(argsList):
	...
	CyInterface().addMessage(iPlayer,True,25,CyTranslator().getText("TXT_KEY_MESSAGE_SLAVE_REVOLT", ()),'',1,', ,Art/Interface/Buttons/Units/Units_atlas4.dds,1,7',ColorTypes(8),pUnit.getX(),pUnit.getY(),True,True)
...
#Sephi
	for i in range(gc.getNumTraitInfos()):
		if (pPlayer.hasTrait(i) and i != gc.getInfoTypeForString('TRAIT_INSANE')):
			if i!=gc.getInfoTypeForString('TRAIT_ENGINEER'):
...
def doVolcanoCreation(argsList):
	kTriggeredData = argsList[0]
	pPlot = gc.getMap().plot(kTriggeredData.iPlotX, kTriggeredData.iPlotY)
	pPlot.setImprovementType(gc.getInfoTypeForString('IMPROVEMENT_VOLCANO'))
	point = pPlot.getPoint()
	CyEngine().triggerEffect(gc.getInfoTypeForString('EFFECT_ARTILLERY_SHELL_EXPLODE'),point)
	CyAudioGame().Play3DSound("AS3D_UN_GRENADE_EXPLODE",point.x,point.y,point.z)
...
def doWerewolf1(argsList):
	...
	if pPlot != -1:
		...
		CyInterface().addMessage(iPlayer,True,25,CyTranslator().getText("TXT_KEY_MESSAGE_WEREWOLF_RELEASED", ()),'',1,', ,Art/Interface/Buttons/Units/Units_atlas4.dds,4,14',ColorTypes(7),pPlot.getX(),pPlot.getY(),True,True)
...
def doWerewolf3(argsList):
	...
	pPlayer = gc.getPlayer(iPlayer)
	pCity = pPlayer.getCity(kTriggeredData.iCityId)
	CyInterface().addMessage(iPlayer,True,25,CyTranslator().getText("TXT_KEY_MESSAGE_WEREWOLF_KILLED", ()),'',1,', ,Art/Interface/Buttons/Units/Units_atlas4.dds,4,14',ColorTypes(8),pCity.getX(),pCity.getY(),True,True)
...
def canDoKhazadForge1(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)
	pCity = pPlayer.getCity(kTriggeredData.iCityId)
	...
	if pCity.isHasBuilding(iForge):
		return False

	return True

def canDoKhazadForge2(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)
	pCity = pPlayer.getCity(kTriggeredData.iCityId)
...
def canApplyHurricane1(argsList):
	...
	listBuildings = []
	for iBuilding in range(gc.getNumBuildingInfos()):
		if (city.getNumRealBuilding(iBuilding) > 0 and (not gc.getBuildingInfo(iBuilding).isDistrict()) and gc.getBuildingInfo(iBuilding).getProductionCost() > 0 and not isLimitedWonderClass(gc.getBuildingInfo(iBuilding).getBuildingClassType())):
			listBuildings.append(iBuilding)
...
def applyHurricane1(argsList):
	...
	for iBuilding in range(gc.getNumBuildingInfos()):
		if (city.getNumRealBuilding(iBuilding) > 0 and (not gc.getBuildingInfo(iBuilding).isDistrict()) and gc.getBuildingInfo(iBuilding).getProductionCost() <= 100 and gc.getBuildingInfo(iBuilding).getProductionCost() > 0 and not isLimitedWonderClass(gc.getBuildingInfo(iBuilding).getBuildingClassType())):
...
#added Callbacks for FFH Events (AI)

def AI_canDoEvilAct(argsList):
	kTriggeredData = argsList[1]
	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)
...
def canTriggerOppertunist1(argsList):
	if gc.getGame().getElapsedGameTurns() == 1:
		return true
	return false

def canTriggerOppertunist2(argsList):
	if gc.getGame().getElapsedGameTurns() == 2:
		return true
	return false
...
def doAddSummoner(argsList):
	...
	iPlayer = kTriggeredData.ePlayer
	pPlayer = gc.getPlayer(iPlayer)
	pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_SUMMONER'),True)
	pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_MINOR'),False)
...
def doAddDefender(argsList):
	...
	iPlayer = kTriggeredData.ePlayer
	pPlayer = gc.getPlayer(iPlayer)
	pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_DEFENDER'),True)
	pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_MINOR'),False)
...
def doAddSwashbuckler(argsList):
	...
	iPlayer = kTriggeredData.ePlayer
	pPlayer = gc.getPlayer(iPlayer)
	pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_SWASHBUCKLER'),True)
	pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_MINOR'),False)
...
def doAddIngenuity(argsList):
	...
	iPlayer = kTriggeredData.ePlayer
	pPlayer = gc.getPlayer(iPlayer)
	pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_INGENUITY'),True)
...
def doAddMagicResistant(argsList):
	...
	iPlayer = kTriggeredData.ePlayer
	pPlayer = gc.getPlayer(iPlayer)
	pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_MAGIC_RESISTANT'),True)
...
def doElectionSupportHawk(argsList):
	...
	if CyGame().getSorenRandNum(100, "Election") < 20:
		CyInterface().addMessage(iPlayer,True,25,CyTranslator().getText("TXT_KEY_EVENT_REPUBLIC_ELECTION_OPPOSITION_PARTY_WINS", ()),'',1,'Art/Interface/Buttons/Civics/Republic.dds',gc.getInfoTypeForString("COLOR_RED"),pPlayer.getCapitalCity().getX(),pPlayer.getCapitalCity().getY(),True,True)
		return

	if pPlayer.hasTrait(gc.getInfoTypeForString('TRAIT_AGGRESSIVE')):
		CyInterface().addMessage(iPlayer,True,25,CyTranslator().getText("TXT_KEY_EVENT_REPUBLIC_ELECTION_HAWK_WINS_ALREADY_AGGRESSIVE", ()),'',1,'Art/Interface/Buttons/Civics/Republic.dds',gc.getInfoTypeForString("COLOR_GREEN"),pPlayer.getCapitalCity().getX(),pPlayer.getCapitalCity().getY(),True,True)
		pPlayer.initUnit(gc.getInfoTypeForString('UNIT_COMMANDER'), pPlayer.getCapitalCity().getX(), pPlayer.getCapitalCity().getY(), UnitAITypes.NO_UNITAI, DirectionTypes.DIRECTION_SOUTH)

	else:
		CyInterface().addMessage(iPlayer,True,25,CyTranslator().getText("TXT_KEY_EVENT_REPUBLIC_ELECTION_HAWK_WINS", ()),'',1,'Art/Interface/Buttons/Civics/Republic.dds',gc.getInfoTypeForString("COLOR_GREEN"),pPlayer.getCapitalCity().getX(),pPlayer.getCapitalCity().getY(),True,True)
		pPlayer.setFeatAccomplished(FeatTypes.FEAT_REPUBLIC_GAINED_AGGRESSIVE, True)
		pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_AGGRESSIVE'),True)
	return
...
def doElectionSupportDove(argsList):
	...
	if pPlayer.hasTrait(gc.getInfoTypeForString('TRAIT_DEFENDER')):
		CyInterface().addMessage(iPlayer,True,25,CyTranslator().getText("TXT_KEY_EVENT_REPUBLIC_ELECTION_DOVE_WINS_ALREADY_DEFENSIVE", ()),'',1,'Art/Interface/Buttons/Civics/Republic.dds',gc.getInfoTypeForString("COLOR_GREEN"),pPlayer.getCapitalCity().getX(),pPlayer.getCapitalCity().getY(),True,True)
		for iLoopPlayer in range(gc.getMAX_CIV_PLAYERS()):
			loopPlayer = gc.getPlayer(iLoopPlayer)
			if loopPlayer.isAlive():
				if loopPlayer.getTeam() != pPlayer.getTeam():
		...
		CyInterface().addMessage(iPlayer,True,25,CyTranslator().getText("TXT_KEY_EVENT_REPUBLIC_ELECTION_DOVE_WINS", ()),'',1,'Art/Interface/Buttons/Civics/Republic.dds',gc.getInfoTypeForString("COLOR_GREEN"),pPlayer.getCapitalCity().getX(),pPlayer.getCapitalCity().getY(),True,True)
		pPlayer.setFeatAccomplished(FeatTypes.FEAT_REPUBLIC_GAINED_DEFENDER, True)
		pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_DEFENDER'),True)
	return
...
def doElectionSupportLandOwner(argsList):
	...
	if pPlayer.hasTrait(gc.getInfoTypeForString('TRAIT_FINANCIAL')):
		CyInterface().addMessage(iPlayer,True,25,CyTranslator().getText("TXT_KEY_EVENT_REPUBLIC_ELECTION_LANDOWNER_WINS_ALREADY_FINANCIAL", ()),'',1,'Art/Interface/Buttons/Civics/Republic.dds',gc.getInfoTypeForString("COLOR_GREEN"),pPlayer.getCapitalCity().getX(),pPlayer.getCapitalCity().getY(),True,True)
		pPlayer.initUnit(gc.getInfoTypeForString('UNIT_MERCHANT'), pPlayer.getCapitalCity().getX(), pPlayer.getCapitalCity().getY(), UnitAITypes.NO_UNITAI, DirectionTypes.DIRECTION_SOUTH)
	else:
		CyInterface().addMessage(iPlayer,True,25,CyTranslator().getText("TXT_KEY_EVENT_REPUBLIC_ELECTION_LANDOWNER_WINS", ()),'',1,'Art/Interface/Buttons/Civics/Republic.dds',gc.getInfoTypeForString("COLOR_GREEN"),pPlayer.getCapitalCity().getX(),pPlayer.getCapitalCity().getY(),True,True)
		pPlayer.setFeatAccomplished(FeatTypes.FEAT_REPUBLIC_GAINED_FINANCIAL, True)
		pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_FINANCIAL'),True)
	return
...
def doElectionSupportPeasant(argsList):
	...
	if pPlayer.hasTrait(gc.getInfoTypeForString('TRAIT_EXPANSIVE')):
		CyInterface().addMessage(iPlayer,True,25,CyTranslator().getText("TXT_KEY_EVENT_REPUBLIC_ELECTION_PEASANT_WINS_ALREADY_EXPANSIVE", ()),'',1,'Art/Interface/Buttons/Civics/Republic.dds',gc.getInfoTypeForString("COLOR_GREEN"),pPlayer.getCapitalCity().getX(),pPlayer.getCapitalCity().getY(),True,True)
		(loopCity, iter) = pPlayer.firstCity(false)
		while(loopCity):
			loopCity.changeHappinessTimer(30)
		...
		CyInterface().addMessage(iPlayer,True,25,CyTranslator().getText("TXT_KEY_EVENT_REPUBLIC_ELECTION_PEASANT_WINS", ()),'',1,'Art/Interface/Buttons/Civics/Republic.dds',gc.getInfoTypeForString("COLOR_GREEN"),pPlayer.getCapitalCity().getX(),pPlayer.getCapitalCity().getY(),True,True)
		pPlayer.setFeatAccomplished(FeatTypes.FEAT_REPUBLIC_GAINED_EXPANSIVE, True)
		pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_EXPANSIVE'),True)
	return
...
def doElectionSupportChurch(argsList):
	...
	if pPlayer.hasTrait(gc.getInfoTypeForString('TRAIT_SPIRITUAL')):
		CyInterface().addMessage(iPlayer,True,25,CyTranslator().getText("TXT_KEY_EVENT_REPUBLIC_ELECTION_CHURCH_WINS_ALREADY_SPIRITUAL", ()),'',1,'Art/Interface/Buttons/Civics/Republic.dds',gc.getInfoTypeForString("COLOR_GREEN"),pPlayer.getCapitalCity().getX(),pPlayer.getCapitalCity().getY(),True,True)
		pPlayer.initUnit(gc.getInfoTypeForString('UNIT_PROPHET'), pPlayer.getCapitalCity().getX(), pPlayer.getCapitalCity().getY(), UnitAITypes.NO_UNITAI, DirectionTypes.DIRECTION_SOUTH)
	else:
		CyInterface().addMessage(iPlayer,True,25,CyTranslator().getText("TXT_KEY_EVENT_REPUBLIC_ELECTION_CHURCH_WINS", ()),'',1,'Art/Interface/Buttons/Civics/Republic.dds',gc.getInfoTypeForString("COLOR_GREEN"),pPlayer.getCapitalCity().getX(),pPlayer.getCapitalCity().getY(),True,True)
		pPlayer.setFeatAccomplished(FeatTypes.FEAT_REPUBLIC_GAINED_SPIRITUAL, True)
		pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_SPIRITUAL'),True)
	return
...
def doElectionSupportState(argsList):
	...
	if pPlayer.hasTrait(gc.getInfoTypeForString('TRAIT_ORGANIZED')):
		CyInterface().addMessage(iPlayer,True,25,CyTranslator().getText("TXT_KEY_EVENT_REPUBLIC_ELECTION_STATE_WINS_ALREADY_ORGANIZED", ()),'',1,'Art/Interface/Buttons/Civics/Republic.dds',gc.getInfoTypeForString("COLOR_GREEN"),pPlayer.getCapitalCity().getX(),pPlayer.getCapitalCity().getY(),True,True)
	else:
		CyInterface().addMessage(iPlayer,True,25,CyTranslator().getText("TXT_KEY_EVENT_REPUBLIC_ELECTION_STATE_WINS", ()),'',1,'Art/Interface/Buttons/Civics/Republic.dds',gc.getInfoTypeForString("COLOR_GREEN"),pPlayer.getCapitalCity().getX(),pPlayer.getCapitalCity().getY(),True,True)
		pPlayer.setFeatAccomplished(FeatTypes.FEAT_REPUBLIC_GAINED_ORGANIZED, True)
		pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_ORGANIZED'),True)
	return
...
def doElectionSupportLabor(argsList):
	...
	if pPlayer.hasTrait(gc.getInfoTypeForString('TRAIT_INDUSTRIOUS')):
		CyInterface().addMessage(iPlayer,True,25,CyTranslator().getText("TXT_KEY_EVENT_REPUBLIC_ELECTION_LABOR_WINS_ALREADY_INDUSTRIOUS", ()),'',1,'Art/Interface/Buttons/Civics/Republic.dds',gc.getInfoTypeForString("COLOR_GREEN"),pPlayer.getCapitalCity().getX(),pPlayer.getCapitalCity().getY(),True,True)
		pPlayer.initUnit(gc.getInfoTypeForString('UNIT_ENGINEER'), pPlayer.getCapitalCity().getX(), pPlayer.getCapitalCity().getY(), UnitAITypes.NO_UNITAI, DirectionTypes.DIRECTION_SOUTH)
	else:
		CyInterface().addMessage(iPlayer,True,25,CyTranslator().getText("TXT_KEY_EVENT_REPUBLIC_ELECTION_LABOR_WINS", ()),'',1,'Art/Interface/Buttons/Civics/Republic.dds',gc.getInfoTypeForString("COLOR_GREEN"),pPlayer.getCapitalCity().getX(),pPlayer.getCapitalCity().getY(),True,True)
		pPlayer.setFeatAccomplished(FeatTypes.FEAT_REPUBLIC_GAINED_INDUSTRIOUS, True)
		pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_INDUSTRIOUS'),True)
	return
...
def doElectionSupportAcademia(argsList):
	...
	if CyGame().getSorenRandNum(100, "Election") < 20:
		CyInterface().addMessage(iPlayer,True,25,CyTranslator().getText("TXT_KEY_EVENT_REPUBLIC_ELECTION_OPPOSITION_PARTY_WINS", ()),'',1,'Art/Interface/Buttons/Civics/Republic.dds',gc.getInfoTypeForString("COLOR_RED"),pPlayer.getCapitalCity().getX(),pPlayer.getCapitalCity().getY(),True,True)
		pPlayer.initUnit(gc.getInfoTypeForString('UNIT_SCIENTIST'), pPlayer.getCapitalCity().getX(), pPlayer.getCapitalCity().getY(), UnitAITypes.NO_UNITAI, DirectionTypes.DIRECTION_SOUTH)
		return

	if pPlayer.hasTrait(gc.getInfoTypeForString('TRAIT_PHILOSOPHICAL')):
		CyInterface().addMessage(iPlayer,True,25,CyTranslator().getText("TXT_KEY_EVENT_REPUBLIC_ELECTION_ACADEMIA_WINS_ALREADY_PHILOSOPHICAL", ()),'',1,'Art/Interface/Buttons/Civics/Republic.dds',gc.getInfoTypeForString("COLOR_GREEN"),pPlayer.getCapitalCity().getX(),pPlayer.getCapitalCity().getY(),True,True)
	else:
		CyInterface().addMessage(iPlayer,True,25,CyTranslator().getText("TXT_KEY_EVENT_REPUBLIC_ELECTION_ACADEMIA_WINS", ()),'',1,'Art/Interface/Buttons/Civics/Republic.dds',gc.getInfoTypeForString("COLOR_GREEN"),pPlayer.getCapitalCity().getX(),pPlayer.getCapitalCity().getY(),True,True)
		pPlayer.setFeatAccomplished(FeatTypes.FEAT_REPUBLIC_GAINED_PHILOSOPHICAL, True)
		pPlayer.setHasTrait(gc.getInfoTypeForString('TRAIT_PHILOSOPHICAL'),True)
	return
...
# Scions start - "Pop" the minor heroes when the appropraite tech is gained.
def doPopAlcinus(argsList):
	kTriggeredData = argsList[0]
	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)
	pTomb = pPlayer.getCapitalCity()
	if not CyGame().isUnitClassMaxedOut(gc.getInfoTypeForString('UNITCLASS_ALCINUS'), 0):
...
def doPopPelemoc(argsList):
	...
	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)
	pTomb = pPlayer.getCapitalCity()
	if not CyGame().isUnitClassMaxedOut(gc.getInfoTypeForString('UNITCLASS_PELEMOC'), 0):
		newUnit = pPlayer.initUnit(gc.getInfoTypeForString('UNIT_PELEMOC'), pTomb.getX(), pTomb.getY(), UnitAITypes.NO_UNITAI, DirectionTypes.DIRECTION_SOUTH)

def doPopThemoch(argsList):
	kTriggeredData = argsList[0]
	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)
	pTomb = pPlayer.getCapitalCity()
	if not CyGame().isUnitClassMaxedOut(gc.getInfoTypeForString('UNITCLASS_THEMOCH'), 0):
		newUnit = pPlayer.initUnit(gc.getInfoTypeForString('UNIT_THEMOCH'), pTomb.getX(), pTomb.getY(), UnitAITypes.NO_UNITAI, DirectionTypes.DIRECTION_SOUTH)

def doPopMelante(argsList):
	kTriggeredData = argsList[0]
	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)
	pTomb = pPlayer.getCapitalCity()
	if not CyGame().isUnitClassMaxedOut(gc.getInfoTypeForString('UNITCLASS_MELANTE'), 0):
...
def doPopKorrina(argsList):
	...
	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)
	pTomb = pPlayer.getCapitalCity()
	if not CyGame().isUnitClassMaxedOut(gc.getInfoTypeForString('UNITCLASS_KORRINA'), 0):
		newUnit = pPlayer.initUnit(gc.getInfoTypeForString('UNIT_KORRINA_PROTECTOR'), pTomb.getX(), pTomb.getY(), UnitAITypes.NO_UNITAI, DirectionTypes.DIRECTION_SOUTH)

def doPopEmperor(argsList):
	kTriggeredData = argsList[0]
	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)
	pTomb = pPlayer.getCapitalCity()
	if not CyGame().isUnitClassMaxedOut(gc.getInfoTypeForString('UNITCLASS_RISEN_EMPEROR'), 0):
		newUnit = pPlayer.initUnit(gc.getInfoTypeForString('UNIT_RISEN_EMPEROR'), pTomb.getX(), pTomb.getY(), UnitAITypes.NO_UNITAI, DirectionTypes.DIRECTION_SOUTH)
...
# Start Houses of Erebus

def canAISpreadHouse(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	...
	for i in range(gc.getNumCorporationInfos()):
		if pPlayer.isHasCorporationSupport(i):
			return false
	return true

def doSpreadHouseGhallanda(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]

	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)

	iGuildHouse = gc.getInfoTypeForString('BUILDING_HOUSE_GHALLANDA')
	pBestCity = -1
	iBestValue = 0
	iValue = 0

	pPlayer.setHasCorporationSupport(gc.getInfoTypeForString('CORPORATION_HOUSE_GHALLANDA'),true)
	pPlayer.changeCorporationSupport(gc.getInfoTypeForString('CORPORATION_HOUSE_GHALLANDA'),10000)

	pPlayer.getCapitalCity().setNumRealBuilding(iGuildHouse,1)
	pPlayer.getCapitalCity().setHasCorporation(gc.getInfoTypeForString('CORPORATION_HOUSE_GHALLANDA'),true,true,false)

def doSpreadHousePhiarlan(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]

	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)

	iGuildHouse = gc.getInfoTypeForString('BUILDING_HOUSE_PHIARLAN')
	pBestCity = -1
	iBestValue = 0
	iValue = 0

	pPlayer.setHasCorporationSupport(gc.getInfoTypeForString('CORPORATION_HOUSE_PHIARLAN'),true)
	pPlayer.changeCorporationSupport(gc.getInfoTypeForString('CORPORATION_HOUSE_PHIARLAN'),10000)

	pPlayer.getCapitalCity().setNumRealBuilding(iGuildHouse,1)
	pPlayer.getCapitalCity().setHasCorporation(gc.getInfoTypeForString('CORPORATION_HOUSE_PHIARLAN'),true,true,false)

def doSpreadHouseKundarak(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]

	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)

	iGuildHouse = gc.getInfoTypeForString('BUILDING_HOUSE_KUNDARAK')
	pBestCity = -1
	iBestValue = 0
	iValue = 0

	pPlayer.setHasCorporationSupport(gc.getInfoTypeForString('CORPORATION_HOUSE_KUNDARAK'),true)
	pPlayer.changeCorporationSupport(gc.getInfoTypeForString('CORPORATION_HOUSE_KUNDARAK'),10000)

	pPlayer.getCapitalCity().setNumRealBuilding(iGuildHouse,1)
	pPlayer.getCapitalCity().setHasCorporation(gc.getInfoTypeForString('CORPORATION_HOUSE_KUNDARAK'),true,true,false)

def doSpreadHouseTharashk(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]

	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)

	iGuildHouse = gc.getInfoTypeForString('BUILDING_HOUSE_THARASHK')
	pBestCity = -1
	iBestValue = 0
	iValue = 0

	pPlayer.setHasCorporationSupport(gc.getInfoTypeForString('CORPORATION_HOUSE_THARASHK'),true)
	pPlayer.changeCorporationSupport(gc.getInfoTypeForString('CORPORATION_HOUSE_THARASHK'),10000)

	pPlayer.getCapitalCity().setNumRealBuilding(iGuildHouse,1)
	pPlayer.getCapitalCity().setHasCorporation(gc.getInfoTypeForString('CORPORATION_HOUSE_THARASHK'),true,true,false)

def doSpreadHouseCannith(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]

	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)

	iGuildHouse = gc.getInfoTypeForString('BUILDING_HOUSE_CANNITH')
	pBestCity = -1
	iBestValue = 0
	iValue = 0

	pPlayer.setHasCorporationSupport(gc.getInfoTypeForString('CORPORATION_HOUSE_CANNITH'),true)
	pPlayer.changeCorporationSupport(gc.getInfoTypeForString('CORPORATION_HOUSE_CANNITH'),10000)

	pPlayer.getCapitalCity().setNumRealBuilding(iGuildHouse,1)
	pPlayer.getCapitalCity().setHasCorporation(gc.getInfoTypeForString('CORPORATION_HOUSE_CANNITH'),true,true,false)

def doSpreadHouseGuildofNine(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]

	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)

	iGuildHouse = gc.getInfoTypeForString('BUILDING_GUILD_OF_THE_NINE')
	pBestCity = -1
	iBestValue = 0
	iValue = 0

	pPlayer.setHasCorporationSupport(gc.getInfoTypeForString('CORPORATION_GUILD_OF_THE_NINE'),true)
	pPlayer.changeCorporationSupport(gc.getInfoTypeForString('CORPORATION_GUILD_OF_THE_NINE'),10000)

	pPlayer.getCapitalCity().setNumRealBuilding(iGuildHouse,1)
	pPlayer.getCapitalCity().setHasCorporation(gc.getInfoTypeForString('CORPORATION_GUILD_OF_THE_NINE'),true,true,false)

def doSpreadHouseVadalis(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]

	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)

	iGuildHouse = gc.getInfoTypeForString('BUILDING_HOUSE_VADALIS')
	pBestCity = -1
	iBestValue = 0
	iValue = 0

	pPlayer.setHasCorporationSupport(gc.getInfoTypeForString('CORPORATION_HOUSE_VADALIS'),true)
	pPlayer.changeCorporationSupport(gc.getInfoTypeForString('CORPORATION_HOUSE_VADALIS'),10000)

	pPlayer.getCapitalCity().setNumRealBuilding(iGuildHouse,1)
	pPlayer.getCapitalCity().setHasCorporation(gc.getInfoTypeForString('CORPORATION_HOUSE_VADALIS'),true,true,false)

def canTriggerNobleHousesFirstWarning(argsList):
	kTriggeredData = argsList[0]
	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)

	return false

def canTriggerNobleHousesSecondWarning(argsList):
	kTriggeredData = argsList[0]
	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)
	...
	#New Esus Event
def joinEsusCult(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)

	#Block the Cult of Esus for the AI for now
	if not pPlayer.isHuman():
		return

	pPlayer.setFollowEsus(true)
	iReligion = pPlayer.getStateReligion()
	iEmpy = gc.getInfoTypeForString('RELIGION_THE_EMPYREAN')
	iEsus = gc.getInfoTypeForString('RELIGION_COUNCIL_OF_ESUS')
	if iReligion == iEmpy:
		pPlayer.setLastStateReligion(ReligionTypes.NO_RELIGION)

#Devine Units
	py = PyPlayer(kTriggeredData.ePlayer)

	for pUnit in py.getUnitList():
		if pUnit.getReligion() == iEmpy:
			pUnit.kill(False, PlayerTypes.NO_PLAYER)
			continue
		if pUnit.getUnitCombatType() == gc.getInfoTypeForString('UNITCOMBAT_DISCIPLE'):
			if CyGame().getSorenRandNum(100, "Assasination") < 50:
				pUnit.kill(False, PlayerTypes.NO_PLAYER)
				continue
		if pUnit.getUnitCombatType() == gc.getInfoTypeForString('UNITCOMBAT_RECON'):
			pUnit.setReligion(iEsus)

#Local Legends
def spawnLocalLegend(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)
	...
	if iCiv==gc.getInfoTypeForString('CIVILIZATION_HIPPUS')	or iCiv==gc.getInfoTypeForString('CIVILIZATION_KURIOTATES') or iCiv==gc.getInfoTypeForString('CIVILIZATION_MALAKIM'):
		iUnitClass=gc.getInfoTypeForString('UNITCLASS_HORSEMAN')

	if iCiv==gc.getInfoTypeForString('CIVILIZATION_SHEAIM')	or iCiv==gc.getInfoTypeForString('CIVILIZATION_AMURITES'):
		iUnitClass=gc.getInfoTypeForString('UNITCLASS_ADEPT')

	if iCiv==gc.getInfoTypeForString('CIVILIZATION_BANNOR')	or iCiv==gc.getInfoTypeForString('CIVILIZATION_CALABIM') or iCiv==gc.getInfoTypeForString('CIVILIZATION_FAERIES') or iCiv==gc.getInfoTypeForString('CIVILIZATION_KHAZAD'):
		iUnitClass=gc.getInfoTypeForString('UNITCLASS_AXEMAN')

	if iCiv==gc.getInfoTypeForString('CIVILIZATION_LUCHUIRP'):
		iUnitClass=gc.getInfoTypeForString('UNITCLASS_WOODGOLEM')

	if iCiv==gc.getInfoTypeForString('CIVILIZATION_LJOSALFAR'):
		iUnitClass=gc.getInfoTypeForString('UNITCLASS_ARCHER')

	if iCiv==gc.getInfoTypeForString('CIVILIZATION_ARISTRAKH'):
		iUnitClass=gc.getInfoTypeForString('UNITCLASS_UD_GENERAL')

	if iCiv==gc.getInfoTypeForString('CIVILIZATION_FAERIES'):
		iUnitClass=gc.getInfoTypeForString('UNITCLASS_SPRITE')
	...
	if gc.getCivilizationInfo(iCiv).getDefaultUnit() != -1:
		iUnitType = gc.getCivilizationInfo(iCiv).getDefaultUnit()

	if iUnitType!=-1:
		newUnit = pPlayer.initUnit(iUnitType, pCity.getX(), pCity.getY(), UnitAITypes.UNITAI_HERO, DirectionTypes.DIRECTION_NORTH)
		newUnit.setExperience(50, -1)
		if not CyGame().isGameMultiPlayer():
			sname = getRandomName()
			sname += ", Local Legend of "
			sname +=pCity.getName()
			newUnit.setName(sname)

		if iCiv==gc.getInfoTypeForString('CIVILIZATION_GRIGORI'):
			newUnit.setHasPromotion(gc.getInfoTypeForString('PROMOTION_HERO'),true)
		if iCiv==gc.getInfoTypeForString('CIVILIZATION_HIPPUS')	or iCiv==gc.getInfoTypeForString('CIVILIZATION_SIDAR') or iCiv==gc.getInfoTypeForString('CIVILIZATION_SVARTALFAR'):
			newUnit.setHasPromotion(gc.getInfoTypeForString('PROMOTION_HIDDEN_NATIONALITY'), true)
		if iCiv==gc.getInfoTypeForString('CIVILIZATION_BALSERAPHS'):
			newUnit.setHasPromotion(gc.getInfoTypeForString('PROMOTION_MUTATED'), true)

def retireLocalLegend(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)

	iPromLL=gc.getInfoTypeForString('PROMOTION_LOCAL_LEGEND')

	for pUnit in PyPlayer(kTriggeredData.ePlayer).getUnitList():
		if pUnit.isHasPromotion(iPromLL):
#			CyInterface().addMessage(pUnit.getOwner(),true,25,"After serving you for many years, %s decides to retire."%(pUnit.getName()),'',0,'',ColorTypes(11), pUnit.getX(), pUnit.getY(), True,True)
			pUnit.kill(true,-1)
			pPlayer.getCapitalCity().setNumRealBuilding(gc.getInfoTypeForString('BUILDING_LOCAL_LEGEND'), 1)
			return

def dealLocalLegend(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)
...
#			CyInterface().addMessage(pUnit.getOwner(),true,25,"After serving you for many years, %s decides to retire."%(pUnit.getName()),'',0,'',ColorTypes(11), pUnit.getX(), pUnit.getY(), True,True)
			pUnit.setDuration(0)
			pPlayer.getCapitalCity().setNumRealBuilding(gc.getInfoTypeForString('BUILDING_LOCAL_LEGEND_DEAL'), 1)
			return

def doMercenariesSupport(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]

	iTeam = CyGame().getMercenariesWarTarget()

	bValid=true
	for i in range(gc.getMAX_CIV_PLAYERS()):
		if iTeam==gc.getPlayer(i).getTeam():
			if gc.getPlayer(i).getCivilizationType()==gc.getInfoTypeForString('CIVILIZATION_HIPPUS'):
				bValid=false
				break

	if bValid:
		gc.getTeam(gc.getMERCENARIES_TEAM()).declareWar(CyGame().getMercenariesWarTarget(),false,-1)

def applyMagnadineRecruitedNone(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)

	iAdventure=gc.getInfoTypeForString('ADVENTURE_HIPPUS_EPIC2')
	iAdventureStep=-1
	for i in range(gc.getMAX_ADVENTURESTEPS()):
		if gc.getAdventureInfo(iAdventure).getAdventureStep(i)==gc.getInfoTypeForString('ADVENTURESTEP_SPELL_HIRE_UNITS'):
			iAdventureStep=i
			break

	if iAdventureStep!=-1:
		pPlayer.changeAdventureCounter(iAdventure,iAdventureStep,-1)

def canDoEsus(argsList):
	kTriggeredData = argsList[1]
	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)
	return gc.getTeam(pPlayer.getTeam()).isHasTech(gc.getInfoTypeForString('TECH_ESUS_GUILD'))

def doAdvisorEarlyTechMilitary(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)

	if pPlayer.isHuman():
		eWarriorCaste = gc.getInfoTypeForString('TECH_WARRIOR_CASTE')
		if eWarriorCaste != -1:
			pPlayer.pushResearch(eWarriorCaste, true)

def doAdvisorEarlyTechGuild(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)

	if pPlayer.isHuman():
		eTech = gc.getInfoTypeForString('TECH_TAVERNS')
		if eTech != -1:
			pPlayer.pushResearch(eTech, true)

def doAdvisorEarlyTechFestivals(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)

	if pPlayer.isHuman():
		eTech = gc.getInfoTypeForString('TECH_FESTIVALS')
		if eTech != -1:
			pPlayer.pushResearch(eTech, true)

def doAdvisorEarlyTechWriting(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)

	if pPlayer.isHuman():
		eTech = gc.getInfoTypeForString('TECH_WRITING')
		if eTech != -1:
			pPlayer.pushResearch(eTech, true)

def doAdvisorEarlyTechMagic(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)

	if pPlayer.isHuman():
		eTech = gc.getInfoTypeForString('TECH_KNOWLEDGE_OF_THE_ETHER')
		if eTech != -1:
			pPlayer.pushResearch(eTech, true)

def doAdvisorEarlyTechReligion(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)

	if pPlayer.isHuman():
		eTech = gc.getInfoTypeForString('TECH_MYSTICISM')
		if eTech != -1:
			pPlayer.pushResearch(eTech, true)
