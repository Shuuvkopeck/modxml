...
def reqBloodSacrifice(caster):
	pPlot = caster.plot()
	iBloodMage = gc.getInfoTypeForString('PROMOTION_BLOOD_HERO')
	for i in range(pPlot.getNumUnits()):
		pUnit = pPlot.getUnit(i)
		if (caster.getOwner() == pUnit.getOwner() and pUnit.isHasPromotion(iBloodMage)):
			return True
	return False
...
def spellBloodSacrifice(caster):
	pPlot = caster.plot()
	iBloodMage = gc.getInfoTypeForString('PROMOTION_BLOOD_HERO')
	iEmpoweredBlood = gc.getInfoTypeForString('PROMOTION_EMPOWERED_BLOOD')
	for i in range(pPlot.getNumUnits()):
		pUnit = pPlot.getUnit(i)
		if (caster.getOwner() == pUnit.getOwner() and pUnit.isHasPromotion(iBloodMage)):
			pUnit.setHasPromotion(iEmpoweredBlood, True)
			pUnit.changeExperience(2, -1, False, False, False)

def reqDraconicApotheosis(caster):

	if (caster.getUnitType() == gc.getInfoTypeForString('UNIT_RED_DRAGON_ASCENDED')):
		return False
	return True

def spellDraconicApotheosis(caster):
	iX = caster.getX()
	iY = caster.getY()
	pPlayer = gc.getPlayer(caster.getOwner())
	iTeam = pPlayer.getTeam()
	eTeam = gc.getTeam(iTeam)
	newUnit = pPlayer.initUnit(gc.getInfoTypeForString('UNIT_RED_DRAGON_ASCENDED'), caster.getX(), caster.getY(), UnitAITypes.NO_UNITAI, DirectionTypes.DIRECTION_SOUTH)
...
def spellHeal(caster,amount):
	...
	for i in range(pPlot.getNumUnits()):
		...
		if pUnit.isAlive():
			...
			if pUnit.isHasPromotion(gc.getInfoTypeForString('PROMOTION_POISONED')) and pUnit.getDamage() == 0:
		...
		for iiY in range(iY-2, iY+3, 1):
			...
			for i in range(pPlot.getNumUnits()):
				...
				if eTeam.isAtWar(pUnit.getTeam()):
					...
					if pUnit.getRace() == gc.getInfoTypeForString('PROMOTION_UNDEAD'):
						iValue += 20
					if (pUnit.getRace() == gc.getInfoTypeForString('PROMOTION_DEMON') or pUnit.getRace() == gc.getInfoTypeForString('PROMOTION_DEMONIC_ASPECT')):
						iValue += 20
					if (gc.getPlayer(pUnit.getOwner()).getStateReligion() == gc.getInfoTypeForString('RELIGION_THE_ASHEN_VEIL')):
						iValue += 10
					if (gc.getPlayer(pUnit.getOwner()).getStateReligion() == gc.getInfoTypeForString('RELIGION_OCTOPUS_OVERLORDS')):
						iValue += 5
					if (gc.getPlayer(pUnit.getOwner()).getStateReligion() == gc.getInfoTypeForString('RELIGION_THE_ORDER')):
						iValue -= 10
					iValue += (5 * gc.getPlayer(pUnit.getOwner()).getNumTradeableBonuses(gc.getInfoTypeForString('BONUS_MANA_CHAOS')))
			if (iValue > iBestValue):
				iBestValue = iValue
				pBestPlot = pPlot
	if pBestPlot != -1:
		for i in range(pBestPlot.getNumUnits()):
			...
			if eTeam.isAtWar(pUnit.getTeam()):
				...
				if (gc.getPlayer(pUnit.getOwner()).getStateReligion() == gc.getInfoTypeForString('RELIGION_THE_ORDER')):
					iValue -= 10
				iValue += (5 * gc.getPlayer(pUnit.getOwner()).getNumTradeableBonuses(gc.getInfoTypeForString('BONUS_MANA_CHAOS')))
				pUnit.doDamage(iValue, 75, caster, gc.getInfoTypeForString('DAMAGE_HOLY'), true)
		CyEngine().triggerEffect(gc.getInfoTypeForString('EFFECT_EXPLOSION_SPELL'),pBestPlot.getPoint())
