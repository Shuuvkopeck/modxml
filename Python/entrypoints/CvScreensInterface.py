...
import CvMagicScreen
import CvArcaneScreen
# Houses of Erebus
...
def showGuildScreen(argsList):
	guildScreen.interfaceScreen(argsList[0])
	
magicScreen = CvMagicScreen.CvMagicScreen()
def showMagicScreen(argsList):
	if (-1 != CyGame().getActivePlayer()):
		magicScreen.interfaceScreen(argsList[0])

arcaneScreen = CvArcaneScreen.CvArcaneScreen()
def showArcaneScreen(argsList):
	if (-1 != CyGame().getActivePlayer()):
		arcaneScreen.interfaceScreen(argsList[0])		
# Houses of Erebus

techChooser = CvTechChooser.CvTechChooser()
def showTechChooser(argsList):
	if (-1 != CyGame().getActivePlayer()):
		techChooser.interfaceScreen(argsList[0])
...
def pediaShowHistorical(argsList):
	if (bUsingSevopedia):
		...
		elif (argsList[0] == CivilopediaPageTypes.CIVILOPEDIA_PAGE_CONCEPT_WILDMANA):
			pediaMainScreen.pediaJump(SevoScreenEnums.PEDIA_WILDMANA_CONCEPTS, argsList[1], True, False)
		elif (argsList[0] == CivilopediaPageTypes.CIVILOPEDIA_PAGE_WILDMANA_GUIDE):
			pediaMainScreen.pediaJump(SevoScreenEnums.PEDIA_WILDMANA_GUIDES, argsList[1], True, False)
		else:
