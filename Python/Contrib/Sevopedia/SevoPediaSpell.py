...
#		self.Y_PLAYER_REQS = self.Y_REQUIRES - self.H_PLAYER_REQS - Y_MERGIN
	...
	def placeRequires(self):
		...
		ePromo = gc.getSpellInfo(self.iSpell).getPromotionPrereq3()
		if (ePromo != PromotionTypes.NO_PROMOTION):
			screen.attachImageButton( panelName, "", gc.getPromotionInfo(ePromo).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_PROMOTION, ePromo, 1, False )

		ePromo = gc.getSpellInfo(self.iSpell).getPromotionPrereq4()
		if (ePromo != PromotionTypes.NO_PROMOTION):
			screen.attachImageButton( panelName, "", gc.getPromotionInfo(ePromo).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_PROMOTION, ePromo, 1, False )
	...
	def placeObsoletes(self):
		screen = self.top.getScreen()
		panelName = self.top.getNextWidgetName()
		screen.addPanel( panelName, localText.getText("TXT_KEY_PEDIA_SPELL_OBSOLETES", ()), "", false, true, self.X_OBSOLETES, self.Y_OBSOLETES, self.W_OBSOLETES, self.H_OBSOLETES, PanelStyles.PANEL_STYLE_BLUE50 )
		screen.attachLabel(panelName, "", "  ")

		for i in range(gc.getNumSpellInfos()):
			if(gc.getSpellInfo(i).isObsoletedBySpell(self.iSpell)):
				screen.attachImageButton(panelName, "", gc.getSpellInfo(i).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_SPELL, i, 1, False)
	
		panelName = self.top.getNextWidgetName()
		screen.addPanel( panelName, localText.getText("TXT_KEY_PEDIA_SPELL_OBSOLETED", ()), "", false, true, self.X_OBSOLETED, self.Y_OBSOLETED, self.W_OBSOLETED, self.H_OBSOLETED, PanelStyles.PANEL_STYLE_BLUE50 )
		screen.attachLabel(panelName, "", "  ")

		for i in range(gc.getNumSpellInfos()):
			if(gc.getSpellInfo(self.iSpell).isObsoletedBySpell(i)):
				screen.attachImageButton(panelName, "", gc.getSpellInfo(i).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_SPELL, i, 1, False)
