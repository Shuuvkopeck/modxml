...
##--------	BUGFfH: End Add
		...
		#Terrain Requirement
		for iTerrainPrereq in range(gc.getNumTerrainInfos()):	
			if(gc.getImprovementInfo(self.iImprovement).getTerrainMakesValid(iTerrainPrereq)):
				screen.attachImageButton( panelName, "", gc.getTerrainInfo(iTerrainPrereq).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_TERRAIN, iTerrainPrereq, 1, False )			
