...
#			screen.attachImageButton( panelName, "", gc.getTraitInfo(iTrait).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_TRAIT, iTrait, -1, False )
			screen.attachImageButton( panelName, "", gc.getTechInfo(gc.getInfoTypeForString('TECH_NEVER')).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_TRAIT, iTrait, -1, False )
...
##--------	BUGFfH: End Add
		...
		iPrereq = gc.getBuildingInfo(self.iBuilding).getPrereqCiv()
		if (iPrereq >= 0):
			screen.attachImageButton( panelName, "", gc.getCivilizationInfo(iPrereq).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_CIV, iPrereq, -1, False )
...
##--------	BUGFfH: Modified by Denev 2009/10/08
	...
	def getBuildingType(self, iBuilding):
		...
		if gc.getBuildingInfo(iBuilding).getBuildingClassType()==iPalace:
			return SevoScreenEnums.TYPE_PALACE		
		if gc.getBuildingInfo(iBuilding).isDistrict():
			return SevoScreenEnums.TYPE_DISTRICT					
		if (isWorldWonderClass(gc.getBuildingInfo(iBuilding).getBuildingClassType())):
		...
		if gc.getBuildingInfo(iBuilding).isEnchantment():
			return SevoScreenEnums.TYPE_CITYENCHANTMENT
			
		if not gc.getBuildingInfo(iBuilding).getProductionCost() > 0:
			return -1
