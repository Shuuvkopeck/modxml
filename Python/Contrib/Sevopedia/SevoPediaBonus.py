...
##--------	BUGFfH: End Modify
	...
	def placeAllows(self):
		...
		# add Spellresearch Buttons
		for eLoopTech in range(gc.getNumTechInfos()):
			if (gc.getTechInfo(eLoopTech).getPrereqBonus() == self.iBonus):
				screen.attachImageButton( panelName, "", gc.getTechInfo(eLoopTech).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_TECH, eLoopTech, 1, False )
