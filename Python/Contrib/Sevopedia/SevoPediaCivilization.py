...
##--------	BUGFfH: Added by Denev 2009/08/14
	def placeTrait(self):
		...
		#Favored Improvement
		iFavoredImprovement = gc.getCivilizationInfo(self.iCivilization).getImpInfrastructureHalfCost()
		...
		szPanelTitle = localText.getText("TXT_KEY_CIV_FAVORED_IMPROVEMENT", ())
		if iFavoredImprovement == ImprovementTypes.NO_IMPROVEMENT:
			if len(ltTerrainModifier) > 0:
				szPanelTitle = localText.getText("TXT_KEY_MISC_CIV_TERRAIN", ())
		screen.addPanel(panelName, szPanelTitle, "", False, True, self.X_CIV_TRAIT, self.Y_CIV_TRAIT, self.W_CIV_TRAIT, self.H_CIV_TRAIT, PanelStyles.PANEL_STYLE_BLUE50)

		#place each buttons
		if iFavoredImprovement != ImprovementTypes.NO_IMPROVEMENT:
			screen.attachImageButton(panelName, "", gc.getImprovementInfo(iFavoredImprovement).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_IMPROVEMENT, iFavoredImprovement, 1, False)
