...
##--------	BUGFfH: Added by Denev 2009/10/07
		...
		liPromoOr.append(gc.getPromotionInfo(self.iPromotion).getPromotionPrereqOr5())
		liPromoOr.append(gc.getPromotionInfo(self.iPromotion).getPromotionPrereqOr6())		
...
##--------	BUGFfH: Added by Denev 2009/08/12 (from CvPediaPromotion.py by Kael)
	...
	def placeSpells(self):
		...
		screen.addPanel(panelName, localText.getText("TXT_KEY_CIVILOPEDIA_HISTORY",  ()), "", False, True, self.X_SPELL_PANE, self.Y_SPELL_PANE, self.W_SPELL_PANE, self.H_SPELL_PANE, PanelStyles.PANEL_STYLE_BLUE50)
		screen.attachLabel(panelName, "", "  ")
		listName = self.top.getNextWidgetName()
		pediaText = gc.getPromotionInfo(self.iPromotion).getCivilopedia()
		screen.addMultilineText(listName, pediaText, self.X_SPELL_PANE+5, self.Y_SPELL_PANE+30, self.W_SPELL_PANE-5, self.H_SPELL_PANE-32, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
...
#			if (iPrereq == self.iPromotion):
#				screen.attachImageButton( panelName, "", gc.getSpellInfo(j).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_SPELL, j, 1, False )
#			iPrereq = gc.getSpellInfo(j).getPromotionPrereq3()
#			if (iPrereq == self.iPromotion):
#				screen.attachImageButton( panelName, "", gc.getSpellInfo(j).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_SPELL, j, 1, False )
#			iPrereq = gc.getSpellInfo(j).getPromotionPrereq4()
#			if (iPrereq == self.iPromotion):
#				screen.attachImageButton( panelName, "", gc.getSpellInfo(j).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_SPELL, j, 1, False )


	def isHeroPromotion(self, iPromotion):
		iHero=gc.getInfoTypeForString('PROMOTION_HERO')
		iPrereq=gc.getPromotionInfo(iPromotion).getPrereqPromotion()
		if iHero==iPromotion or iHero==iPrereq:
			return true
			
		if iPrereq!=-1:
			return self.isHeroPromotion(iPrereq)
			
		iPrereqOr=gc.getPromotionInfo(iPromotion).getPrereqOrPromotion1()
		bValid=false
		if iPrereqOr!=-1:
			bValid=self.isHeroPromotion(iPrereqOr)		
			if bValid:
				return true
				
		iPrereqOr=gc.getPromotionInfo(iPromotion).getPrereqOrPromotion2()				
		if iPrereqOr!=-1:
			bValid=self.isHeroPromotion(iPrereqOr)		
	...
	def getPromotionType(self, iPromotion):
		...
		elif (gc.getPromotionInfo(iPromotion).isGear() or gc.getPromotionInfo(iPromotion).getEquipmentCategory()!=-1):
			return SevoScreenEnums.TYPE_GEAR					
		elif (gc.getPromotionInfo(iPromotion).getMinLevel() < 0):
			return SevoScreenEnums.TYPE_EFFECT
		elif gc.getPromotionInfo(iPromotion).isClassPromotion():
			return SevoScreenEnums.TYPE_CLASS
		elif gc.getPromotionInfo(iPromotion).isDiscipline():
			return SevoScreenEnums.TYPE_DISCIPLINE			
		elif (self.isHeroPromotion(iPromotion)):
			return SevoScreenEnums.TYPE_HEROIC			
		else:
