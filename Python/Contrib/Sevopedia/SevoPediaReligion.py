...
#		self.H_SPECIAL = self.Y_REQUIRES + self.H_REQUIRES - self.Y_SPECIAL
		self.H_SPECIAL = self.Y_REQUIRES - self.Y_SPECIAL
		
		self.X_KNOWLEDGE = self.X_MAIN_PANE + self.W_MAIN_PANE + 10
		self.Y_KNOWLEDGE = self.Y_SPECIAL + self.H_SPECIAL + 10
		self.W_KNOWLEDGE = self.top.R_PEDIA_PAGE - self.X_SPECIAL
		self.H_KNOWLEDGE = self.Y_REQUIRES + self.H_REQUIRES - self.Y_KNOWLEDGE		
...
##--------	BUGFfH: End Add
	...
	def placeMiracles(self):
		screen = self.top.getScreen()
		panelName = self.top.getNextWidgetName()
		screen.addPanel( panelName, localText.getText("TXT_KEY_PEDIA_MIRACLES", ()), "", False, True, self.X_REQUIRES, self.Y_REQUIRES, self.W_REQUIRES, self.H_REQUIRES, PanelStyles.PANEL_STYLE_BLUE50 )
		screen.attachLabel(panelName, "", "  ")
		...
		for iLoopSpell in range(gc.getNumProjectInfos()):
			if gc.getProjectInfo(iLoopSpell).getPrereqReligion() == self.iReligion:
				screen.attachImageButton( panelName, "", gc.getProjectInfo(iLoopSpell).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_PROJECT, iLoopSpell, 1, False )

	def placeSacredKnowledge(self):
		screen = self.top.getScreen()
		panelName = self.top.getNextWidgetName()
		screen.addPanel( panelName, localText.getText("TXT_KEY_PEDIA_SACRED_KNOWLEDGE", ()), "", False, True, self.X_KNOWLEDGE, self.Y_KNOWLEDGE, self.W_KNOWLEDGE, self.H_KNOWLEDGE, PanelStyles.PANEL_STYLE_BLUE50 )
		screen.attachLabel(panelName, "", "  ")
		iTech = gc.getReligionInfo(self.iReligion).getTechPrereq()

		for iLoopSpell in range(gc.getNumTechInfos()):
			if gc.getTechInfo(iLoopSpell).getPrereqReligion() == self.iReligion:
				if gc.getTechInfo(iLoopSpell).isHolyKnowledge():
					screen.attachImageButton( panelName, "", gc.getTechInfo(iLoopSpell).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_TECH, iLoopSpell, 1, False )
