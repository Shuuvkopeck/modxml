...
#---Geändert ENDE ------------------------------------------
	...
	def interfaceScreen(self, iTech):
		...
		if CyGlobalContext().getTechInfo(iTech).isSpellResearch():
			self.interfaceScreen_SpellResearch(iTech)
			return
...
#---Eingefügt ENDE -------------------------------------------------
		...
		if techInfo.isSpellResearch():
			szTech = "SpellResearch: " + szTech
...
#---Eingefügt ENDE ------------------------------------------------		
	...
	def interfaceScreen_SpellResearch(self, iTech):
		...
		bW = 712
		bH = 496
		bX = self.W_SCREEN / 2 - bW / 2 
		bY = 100
		
		graphicW = 400	
		graphicH = 300		
		graphicX = self.W_SCREEN / 2 - graphicW / 2
		graphicY = bY + 100
		
		szPath = techInfo.getPicture()
		
		screen.addDrawControl( "TechSplashBackground", "Art/Interface/SpellResearch/Spellbook.dds", bX, bY, bW, bH, WidgetTypes.WIDGET_GENERAL, -1, -1 )			
		screen.addDrawControl( "TechSplashBackground2", szPath, graphicX, graphicY, graphicW, graphicH, WidgetTypes.WIDGET_GENERAL, -1, -1 )	
		screen.showWindowBackground( False )
		screen.setDimensions(screen.centerX(0), screen.centerY(0), self.W_SCREEN, self.H_SCREEN)
		
		szName = techInfo.getDescription()
		screen.setText( "WidgetSpellName", "TechSplashBackground", szName, CvUtil.FONT_CENTER_JUSTIFY, graphicX + graphicW / 2, graphicY + graphicH + 10, -0.3, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )		
		
		# Exit Button
		screen.setButtonGFC("Exit", localText.getText("TXT_KEY_SCREEN_CONTINUE", ()), "", self.X_EXIT, bY + bH, self.W_EXIT , self.H_EXIT, WidgetTypes.WIDGET_CLOSE_SCREEN, -1, -1, ButtonStyles.BUTTON_STYLE_STANDARD )		
